...
def gpg_sign_file(file_path):
    try:
        result = subprocess.run([
            "gpg", "--batch", "--yes", "--output", file_path + ".asc", "--armor", "--detach-sign", file_path
...
def write_signature_file(hash_value, target_dir, hash_type):
    ...
    gpg_sign_file(sig_path)

def main():
    parser = argparse.ArgumentParser(description="Verify ZIP file structure, compute hash, and generate .sig")
    ...
    if args.target_dir:
        ...
        write_signature_file(computed_hash, args.target_dir, args.hash_type)
